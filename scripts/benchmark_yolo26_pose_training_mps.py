...
"""
YOLO26 Pose PyTorch MPS Training Benchmark
==========================================
Measures pose training performance using PyTorch MPS backend on Apple Silicon.

Usage:
    python benchmark_yolo26_pose_training_mps.py
    python benchmark_yolo26_pose_training_mps.py --models n s      # Specific models only
    python benchmark_yolo26_pose_training_mps.py --epochs 5        # Fewer epochs
    python benchmark_yolo26_pose_training_mps.py --batch 2         # Smaller batch size
    python benchmark_yolo26_pose_training_mps.py --output custom.json

Output:
    ../results/yolo26_pose_mps_training_final.json (default, overridable via --output)
"""
...
def train_model_mps(
    ...
    """Train single pose PyTorch model with MPS backend and measure time.
    ...
    # Ultralytics' end-to-end pose loss moves a float64 ``RLE_WEIGHT`` array to
    # the device; MPS rejects float64. Cast it to float32 so MPS training runs
    # (CPU is unaffected). See _mps_pose_perf_patch for details.
...
def main():
    """Parse CLI args, verify MPS support, train each pose model on MPS, and save benchmark results."""
    logging.basicConfig(level=logging.INFO, format="%(message)s")

    parser = argparse.ArgumentParser(description="YOLO26 Pose PyTorch MPS Training Benchmark")
    parser.add_argument(
        "--models",
        nargs="+",
        default=MODEL_SIZES,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=RESULTS_DIR / "yolo26_pose_mps_training_final.json",
        help="Output JSON path (default: results/yolo26_pose_mps_training_final.json)",
    )
    args = parser.parse_args()
    ensure_runtime_dirs(PROJECT_DIR)

    logger.info("=" * 70)
    logger.info("YOLO26 Pose PyTorch MPS Training Benchmark")
    logger.info("=" * 70)
    ...
    # Warm up MPS once on a 1-epoch throwaway run so the one-time Metal kernel
    # compilation cost is not charged to the first timed model. COCO8-Pose is
    # tiny (~20 iterations), so without this the first model's per-epoch time is
    # dominated by warmup and skews its speedup ratio.
    ...
    final_output = {
        "benchmark": "yolo26_pose_mps_training",
    ...
    logger.info("\n✨ PyTorch MPS pose training benchmark complete!")
