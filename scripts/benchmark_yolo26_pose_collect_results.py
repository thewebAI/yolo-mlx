...
"""
YOLO26 Pose Benchmark Results Collector
=======================================
...
- benchmark_yolo26_pose_inference.py       -> yolo26_pose_inference_three_way.json
- benchmark_yolo26_pose_training_mlx.py    -> yolo26_pose_mlx_training_final.json
- benchmark_yolo26_pose_training_mps.py    -> yolo26_pose_mps_training_final.json
- benchmark_yolo26_pose_training_cpu.py    -> yolo26_pose_cpu_training_final.json

And produces a combined report for analysis and chart generation.

Usage:
    python benchmark_yolo26_pose_collect_results.py
    python benchmark_yolo26_pose_collect_results.py --output custom_output.json
...
INPUT_FILES = {
    "inference": RESULTS_DIR / "yolo26_pose_inference_three_way.json",
    "training_mlx": RESULTS_DIR / "yolo26_pose_mlx_training_final.json",
    "training_mps": RESULTS_DIR / "yolo26_pose_mps_training_final.json",
    "training_cpu": RESULTS_DIR / "yolo26_pose_cpu_training_final.json",
}
    ...
    for backend_name, data in backends:
        ...
        for item in data.get("results", []):
            ...
            result[model_name][backend_name] = {
                ...
                "mAP50_pose": item.get("mAP50_pose", 0),
                "mAP50-95_pose": item.get("mAP50-95_pose", 0),
...
def _pose_map50_for_display(backend: dict) -> float:
    v = backend.get("mAP50_pose", 0)
    if v:
        return float(v)
    return float(backend.get("mAP50", 0))


def _pose_map5095_for_display(backend: dict) -> float:
    v = backend.get("mAP50-95_pose", 0)
    if v:
        return float(v)
...
def print_accuracy_summary(training_data: dict) -> None:
    """Print model keypoint accuracy (mAP) summary table.

    Args:
        training_data: Normalized training data keyed by model name then backend, containing mAP scores.
    """
    logger.info("\n" + "=" * 80)
    logger.info("  ACCURACY SUMMARY (keypoint mAP after training)")
    logger.info("=" * 80)
    ...
    logger.info(
        f"\n{'Model':<14} {'MLX m50_p':<12} {'MLX m95_p':<12} {'MPS m50_p':<12} {'MPS m95_p':<12} {'CPU m50_p':<12} {'CPU m95_p':<12}"
    )
    logger.info("-" * 86)
    ...
    for size in MODEL_SIZES:
        ...
        mlx50 = _pose_map50_for_display(mlx)
        mlx95 = _pose_map5095_for_display(mlx)
        mps50 = _pose_map50_for_display(mps)
        mps95 = _pose_map5095_for_display(mps)
        cpu50 = _pose_map50_for_display(cpu)
        cpu95 = _pose_map5095_for_display(cpu)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="YOLO26 Pose Benchmark Results Collector")
    parser.add_argument(
        "--output",
        type=str,
        default=None,
        help="Output file path (default: ../results/yolo26_pose_benchmark_combined.json)",
    )
    args = parser.parse_args()
    ensure_runtime_dirs(PROJECT_DIR)

    logger.info("=" * 80)
    logger.info("  YOLO26 Pose Benchmark Results Collector")
    logger.info("=" * 80)
    ...
    if not any(raw_data.values()):
        ...
        logger.error("   - benchmark_yolo26_pose_inference.py")
        logger.error("   - benchmark_yolo26_pose_training_mlx.py")
        logger.error("   - benchmark_yolo26_pose_training_mps.py")
        logger.error("   - benchmark_yolo26_pose_training_cpu.py")
        return
    ...
    combined = {
        "benchmark": "YOLO26 Pose Combined Benchmark Results",
    ...
    output_path = (
        Path(args.output) if args.output else RESULTS_DIR / "yolo26_pose_benchmark_combined.json"
    )
