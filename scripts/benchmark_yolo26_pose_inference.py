...
"""
YOLO26 Pose Inference Benchmark - Three-Way Comparison
======================================================
Compares pose inference performance across MLX GPU, PyTorch MPS, and PyTorch CPU backends.

Usage:
    python benchmark_yolo26_pose_inference.py
    python benchmark_yolo26_pose_inference.py --models n s      # Specific models only
    python benchmark_yolo26_pose_inference.py --runs 20         # More timed runs
    python benchmark_yolo26_pose_inference.py --output custom.json

Output:
    ../results/yolo26_pose_inference_three_way.json (default, overridable via --output)
"""
...
def test_mlx_inference(
    ...
    Args:
        ...
        allow_random_mlx: Allow the YAML random-weights fallback when converted
            weights are missing.
    ...
    # Find YAML config in the package (yolo26-pose.yaml, not yolo26n-pose.yaml)
    try:
        import yolo26mlx

        pkg_dir = Path(yolo26mlx.__file__).parent
        yaml_candidates = [
            pkg_dir / "cfg" / "models" / "26" / "yolo26-pose.yaml",
            pkg_dir / "cfg" / "models" / "yolo26-pose.yaml",
        ]
    ...
    try:
        if npz_path.exists():
            ...
            model = YOLO(str(npz_path), task="pose")
            model_source = "npz"
        elif safetensors_path.exists():
            logger.info(f"  Loading {model_name} from safetensors...")
            model = YOLO(str(safetensors_path), task="pose")
            ...
            model = YOLO(str(yaml_path), task="pose")
...
def print_summary(results: dict[str, dict], speedups: dict[str, dict]) -> None:
    ...
    """
    logger.info("\n" + "=" * 80)
    logger.info("  YOLO26 Pose Inference Benchmark Summary")
    logger.info("=" * 80)
...
def main():
    """Run the three-way pose inference benchmark, print results, and save to JSON."""
    logging.basicConfig(level=logging.INFO, format="%(message)s")

    parser = argparse.ArgumentParser(description="YOLO26 Pose Inference Benchmark")
    parser.add_argument(
        "--models",
        nargs="+",
        default=MODEL_SIZES,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=RESULTS_DIR / "yolo26_pose_inference_three_way.json",
        help="Output JSON path (default: results/yolo26_pose_inference_three_way.json)",
    )
    parser.add_argument(
        "--allow-random-mlx",
        action="store_true",
    ...
    logger.info("=" * 70)
    logger.info("  YOLO26 Pose Inference Benchmark - Three-Way Comparison")
    logger.info("=" * 70)
    ...
    output = {
        "benchmark": "YOLO26 Pose Inference - Three-Way Comparison",
